...
@app.on_message(filters.command(["أبراج 🧙‍♂️","برج","ابراج"],""))
async def abrag(c: Client, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton("🧚🏻‍♀️العذراء", callback_data="elazraaa " + str(m.from_user.id))] +
    ...
    await m.reply_text("• مرحبـاً بـك عـزيـزي\n• فـي قسـم ( الابـراج )\n• اضغـط على الازرار لـ اختيـار برجـك 💠", reply_markup=keyboard)


@app.on_callback_query(filters.regex("^elgadee (\\d+)$"))
async def elgadee(c: Client, m: CallbackQuery):

    HUOPVTR_ASAAQ = InlineKeyboardMarkup(inline_keyboard=[
...
• مهنياً :  يدعوك هذا اليوم المليء بالسلبيات إلى عدم التورط في قضايا أكبر منك، وخصوصاً أن رياح التغيير بدأت تعصف باتجاهك"""
    await m.message.reply_text(abrag_text, reply_markup=HUOPVTR_ASAAQ)
